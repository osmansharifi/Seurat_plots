...
DEG_data_dir <- "/share/korflab/home/viki/snRNA-seq-pipeline/DEG_data/EdgeR/M_MUT_and_WT_M_E18_WB/"

# Lists
cell_types <- list("L2_3_IT", "L6", "Sst", "L5", "L4", "Pvalb", "Sncg", "Oligo", "Vip", "Lamp5", "Astro", "Peri", "Endo") 
...
# Subset Seurat object for the cluster
edgeR_obj <- subset(experiment.aggregate, subset = celltype.call == "Non-neuronal")
# Generate a count matrix
counts <- as.matrix(edgeR_obj@assays$RNA@counts)
counts <- counts[Matrix::rowSums(counts >= 1) >= 1, ]
# Subset the meta data for filtered gene/cells
...
cell_EdgeR_DEG <- subset(x = qlf.contrast.all.genes$table, subset = FDR < 0.05)
# Write data to CSV so analysis does not need to be rerun when working with data
write.csv(cell_EdgeR_DEG, file = glue(DEG_data_dir, "Non_neuronal", "_", metadata_info, "_EdgeR_DEG.csv"))