...
# Load in data
load(data_file)
experiment.aggregate.combat
Idents(experiment.aggregate.combat) <- 'celltype.call'

# Prepare data
# Rename "Non-neuronal" as "Non_neuronal" for variable name usage
experiment.aggregate.combat <- RenameIdents(object = experiment.aggregate.combat, 'Non-neuronal' = 'Non_neuronal')
# We want to get rid of the G2M and S phase cells, so subset to keep only G1 cells
experiment.aggregate.combat <- subset(x = experiment.aggregate.combat, subset = cell.cycle == "G1")
# Set mitochondrial threshold to 0.5%
experiment.aggregate.combat <- subset(x = experiment.aggregate.combat, subset = percent.mito <= "0.5")

################################################################################
## Limma Analysis

for (cell_type in cell_types){
  cluster_cell <- subset(experiment.aggregate.combat, idents = cell_type)
