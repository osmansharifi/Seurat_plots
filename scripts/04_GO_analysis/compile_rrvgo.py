		...
		for deg in deg_tools:
			for cell_type in cell_types:
				for time_point in time_points:
					my_path = f'/Users/osman/Desktop/LaSalle_lab/Rett_Data/Differential_expression/males/{meta}/{deg}/{cell_type}/rrvgo_enrichr.xlsx'
					...
					tiss = []
					deg_method = []
					for i in range(len(df["Gene Ontology"])):
...
for meta, deg, cell_type, time_point, tissue_type in zip(meta_folders, deg_tools, cell_types, time_points, tissue_types):
	for meta in meta_folders:
		for deg in deg_tools:
			for cell_type in cell_types:
				for time_point in time_points:
					my_path = f'/Users/osman/Desktop/LaSalle_lab/Rett_Data/Differential_expression/females/{meta}/{deg}/{cell_type}/enrichr.xlsx'
					if not os.path.isfile(my_path):
						print("ERROR: file was not found", my_path)	 
						continue
					print(f'Adding data from {my_path} to data frame')
					# Read in DEG Data
					data = pd.read_excel(my_path, sheet_name = [3], ignore_index=True)
					# Turn the DEG Data into a pandas data frame
					df = pd.DataFrame(data)
					print(df)
					sys.exit()
...
#master_deg_df_males.to_csv("/Users/osman/Documents/GitHub/snRNA-seq-pipeline/scripts/04_GO_analysis/male_GO.csv")
master_deg_df_females.to_csv("/Users/osman/Documents/GitHub/snRNA-seq-pipeline/scripts/04_GO_analysis/female_GO.csv")
