...
## Map human cortex scRNA-seq datasets from 7 different technologies
...
p1 <- DimPlot_scCustom(seurat_object = human_rettcort, group.by = "predicted.cross_species_cluster")
p2 <- DimPlot_scCustom(seurat_object = human_rettcort, label = FALSE)
p1 + p2 + plot_layout(ncol = 2)
p1 <- FeaturePlot_scCustom(seurat_object = human_rettcort, features = 'MECP2')
p2 <- DimPlot_scCustom(seurat_object = human_rettcort, group.by = 'orig.ident')
```
```{r marker genes}
# Run basic FindAllMarkers DE test
all_markers <- FindAllMarkers(object = test_name, verbose = F)
...
Stacked_VlnPlot(seurat_object = test_name, features = top_5, x_lab_rotate = TRUE,
