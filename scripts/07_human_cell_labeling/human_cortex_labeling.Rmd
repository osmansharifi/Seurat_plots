...
hrett_cortex <- RunAzimuth(experiment.aggregate, reference = "humancortexref")
#Normalize data
hrett_cortex <- NormalizeData(hrett_cortex)
Idents(hrett_cortex) <- "predicted.cluster"
```
...
DimPlot_scCustom(seurat_object = test_name, label = FALSE)
p1 <- DimPlot(hrett_cortex, group.by = "predicted.class", label = TRUE, label.size = 3) + NoLegend()
p2 <- DimPlot(hrett_cortex, group.by = "predicted.cluster")
p1 + p2
```
```{r marker genes}
# Run basic FindAllMarkers DE test
all_markers <- FindAllMarkers(object = hrett_cortex, verbose = F)
