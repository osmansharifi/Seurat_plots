...
load("/Users/osman/Desktop/LaSalle_lab/adult_maleects/all_male_mouse_cortex.RData")
...
adult_male <- MetacellsByGroups(
  ...
  group.by = c("celltype.call", "Time_Point", "Condition"), # specify the columns in adult_male@meta.data to group by
...
adult_male <- ConstructNetwork(
  ...
  setDatExpr=FALSE,
  tom_name = 'P30' # name of the topoligical overlap matrix written to disk
)

PlotDendrogram(adult_male, main='P30 hdWGCNA Dendrogram')
...
adult_male$Condition <- as.factor(adult_male$Condition)
...
cur_traits <- c('Condition', 'Time_Point', 'orig.ident')
